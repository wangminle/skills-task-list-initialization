...
| 维度 | 检查内容 |
| --- | --- |
| 结构合规 | 非法 ID 格式（如 `bug-001`）、重复 ID、重复章节、坏表格行、动作/状态枚举、表头是否符合当前 7 列时间模型 |
...
| 非法 ID 格式 | `check` 检出形如 `bug-001`/`BUG-12` 的非法 ID（首列「像 ID 但不符合 `大写前缀-三位数字`」）并报错，不再静默跳过；修正为 `BUG-001` 格式 |
| add 遇非标准表头 | `add` 对表头不符合任一标准 schema 的分区（如列顺序错位的 `ID / 状态 / 动作 / …`）拒绝写入，避免按列数猜测把数据填进错误列；先 `standardize --migrate-schema` 或手工修正表头列顺序后再 `add` |
