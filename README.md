...
- **Tested** — 42 unit tests covering init, add, check, summary, standardize, aliases, reverse aliases, heading-whitespace tolerance, `--fix-only` semantics, maintenance-rule detection, schema-variant detection, timestamp validation, case-insensitive section resolution, and the English locale.
...
| `check` | Validate structure, enums & timestamps |
...
- **经过测试**——42 个单元测试，覆盖 init、add、check、summary、standardize、分区别名、反向别名、标题空白容错、`--fix-only` 语义、维护规则检测、schema 变体检测、时间戳校验、分区别名大小写不敏感与英文 locale。
